#!/usr/bin/env python
"""Build and deploy the MedAssist Lambda package.
...
REGION = "ap-south-1"
PREFIX = "medassist-clinical-copilot-dev-"
...
def build(staging: pathlib.Path) -> bytes:
    ...
    buf = pathlib.Path(tempfile.gettempdir()) / "medassist-lambda.zip"
...
def main() -> int:
    ...
    if args.dry_run:
        out = pathlib.Path(tempfile.gettempdir()) / "medassist-lambda.zip"
        print("dry-run: %s" % out)
        return 0
