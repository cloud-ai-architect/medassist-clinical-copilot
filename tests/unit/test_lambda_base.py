"""Unit tests for MedAssist Lambda plumbing and routing."""
...
class TestRunStage:
    def test_missing_parameter_returns_400(self):
        r = run_stage({}, ["complaint"], lambda d: "never")
        assert r["statusCode"] == 400
        assert "complaint" in json.loads(r["body"])["message"]

    def test_lists_every_missing_parameter(self):
        r = run_stage({}, ["service", "clinical_record"], lambda d: "never")
        msg = json.loads(r["body"])["message"]
        assert "service" in msg and "clinical_record" in msg

    def test_empty_string_counts_as_missing(self):
        r = run_stage({"complaint": ""}, ["complaint"], lambda d: "never")
        assert r["statusCode"] == 400

    def test_success_wraps_result(self):
        r = run_stage({"complaint": "x"}, ["complaint"], lambda d: {"ok": True})
...
class TestInvokeJson:
    ...
    def test_model_error_is_a_medassist_error(self):
        assert issubclass(ModelError, MedAssistError)
