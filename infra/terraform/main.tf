...
module "apigateway" {
  ...
  name_prefix     = local.name_prefix
  api_description = "MedAssist clinical agent API"

  # One route per agent; the orchestrator is reachable too, which it
  # previously was not.
  routes = {
    assist    = module.lambdas.function_arns["orchestrator"]
    triage    = module.lambdas.function_arns["triage"]
    historian = module.lambdas.function_arns["historian"]
    scribe    = module.lambdas.function_arns["scribe"]
    priorauth = module.lambdas.function_arns["priorauth"]
...
# window.MEDASSIST_API_URL and fell back to "https://api.example.com",
...
resource "aws_s3_object" "ui_config" {
  ...
  content = <<-JS
    ...
    window.MEDASSIST_API_URL = "${module.apigateway.api_url}";
    window.MEDASSIST_ENV = "${var.environment}";
  JS
